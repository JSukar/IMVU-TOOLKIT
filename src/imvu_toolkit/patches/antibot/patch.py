...
from imvu_toolkit.zip_utils import newest_backup, restore_from_backup, rewrite_jar, rewrite_zip
...
def entries_from_backup(library, zip_names):
    """Restore selected library.zip members from the newest antibot backup."""
    backup = newest_backup(library, C.BACKUP_PREFIX)
    if not backup:
        return {}
    import zipfile
    ...
    with zipfile.ZipFile(backup, "r") as zin:
        for name in zip_names:
            if name in zin.namelist():
                restored[name] = zin.read(name)
    return restored


def patch_library(library, write_entries):
    merged = dict(write_entries)
    merged.update(
        entries_from_backup(library, C.SESSIONWINDOW_ZIP_ENTRIES)
    )
    return rewrite_zip(
        library,
        C.BACKUP_PREFIX,
